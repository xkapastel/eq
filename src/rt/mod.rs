...
lazy_static! {
    static ref WORD_REGEX: regex::Regex = { regex::Regex::new(WORD_PATTERN).unwrap() };
...
impl Heap {
    ...
    fn sweep(&mut self) -> Result<()> {
        ...
        self.generation += 1;
        println!(
            "[gc] deleted: {} generation: {}",
            nodes_deleted, self.generation
        );
        return Ok(());
    }
...
fn quote(root: Ptr, heap: &Heap, buf: &mut String) -> Result<()> {
    match heap.get_ref(root)? {
        ...
        &Object::Opcode(ref value) => match value {
            Opcode::App => {
                buf.push('a');
            }
            Opcode::Box => {
...
fn reduce(continuation: Ptr, heap: &mut Heap, tab: &Library, mut time_quota: u64) -> Result<Ptr> {
...
impl Thread {
    ...
    fn get_continuation(&mut self, heap: &mut Heap) -> Result<Ptr> {
        let mut xs = heap.new_id()?;
    ...
    fn pop_continuation(&mut self, heap: &mut Heap) -> Result<Ptr> {
        loop {
    ...
    fn get_environment(&mut self, heap: &mut Heap) -> Result<Ptr> {
        let mut xs = heap.new_id()?;
    ...
    fn step(&mut self, heap: &mut Heap, tab: &HashMap<Rc<str>, Ptr>) -> Result<()> {
...
fn iter_nodes<'a, F>(node: &'a comrak::nodes::AstNode<'a>, func: &mut F)
where
...
impl Pod {
    ...
    pub fn from_string(src: &str, space_quota: usize, time_quota: u64) -> Result<Self> {
    ...
    pub fn eval(&mut self, src: &str, time_quota: u64) -> Result<String> {
        ...
        if let Some(data) = POD_INSERT_REGEX.captures(src) {
            ...
            let value = reduce(value, &mut self.heap, &self.tab, time_quota)?;
            ...
            let target = reduce(source, &mut self.heap, &self.tab, time_quota)?;
    ...
    pub fn to_string(&self) -> Result<String> {
        ...
        let mut keys: Vec<Rc<str>> = self.tab.keys().map(|x| x.clone()).collect();
        keys.sort();
